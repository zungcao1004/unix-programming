...
void print_dir(char *directory, int include_hidden, int display_details, int sort_by_size, int sort_by_access_time);
...
void sort_files_by_access_time(const char *directory);
...
int main(int argc, char **argv)
{
    ...
    print_dir(directory, include_hidden, display_details, sort_by_size,sort_by_access_time);
...
void print_dir(char *directory, int include_hidden, int display_details, int sort_by_size, int sort_by_access_time)
{
    ...
    // Trường hợp 1: -A và -S không được gọi
    ...
    // Trường hợp 2: -alut và -S không được gọi
    else if ((include_hidden == 0 && display_details == 0 && sort_by_access_time == 0 && sort_by_modification_time == 0) && sort_by_size == 0)
    {
    ...
    // Trường hợp 3: -alut và -A không được gọi
    else if ((include_hidden == 0 && display_details == 0 && sort_by_access_time == 0 && sort_by_modification_time == 0) && list_all_except_dot == 0)
    {
        if (sort_by_size)
        {
            sort_files_by_size(directory);
        }
        else if (sort_by_access_time)
        {
            sort_files_by_access_time(directory);
        }
    }
...
int compare_access_time(const void *a, const void *b)
{
    ...
    return stat_b.st_atime - stat_a.st_atime; // Sắp xếp theo thời gian truy cập, giảm dần
}

// Hàm sắp xếp danh sách file theo thời gian truy cập
void sort_files_by_access_time(const char *directory)
{
    ...
    // Lọc bỏ các file và thư mục ẩn (bắt đầu bằng dấu '.')
    int visible_files = 0;
    ...
    // Tạo một mảng để lưu trữ các file không ẩn
    ...
    // Sắp xếp
    qsort(visible_files_list, visible_files, sizeof(struct dirent *), compare_access_time);
